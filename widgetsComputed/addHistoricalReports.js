return async function(item){
 try {
     ...
     let months = $getGrid('months')
     let groups = $getGrid('users')
     let rowKeyMth
     let rowKeyGr
     months.filter(elem => {
         if(elem.name===mthI){
             rowKeyMth=elem.rowKey
             return
         }
     })
     groups.filter(elem => {
         if(elem.name=== item.name){
             rowKeyGr=elem.rowKey
         }
     })
...
let paydate = this.paymentDate('December')
 ...
 let earnSale = 0
 items.filter(elem=>{
    if(elem.vendorEmail === item.email && elem.invoiceMonth===mthI) {
console.log(elem)
...
itemsS.filter(element=>{
    ...
    if(element.vendorEmail ===item.email && element.invoiceMonth===mthI) {
