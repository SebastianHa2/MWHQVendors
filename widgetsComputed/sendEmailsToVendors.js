...
console.log(elem.mWRent15)
    // item.mWHQCommission=item.netRentalPrice*item.mWHQCommissionRate
        ...
        let rentalData = {
            ...
            commission: parseFloat(elem.mWHQCommission)?parseFloat(elem.mWHQCommission).toFixed(2):'0.00',
        ...
        netbalTotalS += parseFloat(elem.netBalanceDueToVendor)
        gspSaleS += parseFloat(elem.mWRent15)
...
let vendorTotal = parseFloat(vendorSale) + parseFloat(vendorRental) + parseFloat(vendorSub)  
    let payload = {
        mth:mthI,
        vendorName: vendorNameInv,
        paymentDate: paydate,
        earnRent:perRental + "%",
        earnSale:perRentalS + "%",
        invoiceNum:1,
        paymentPeriod:payPeriod,
        email: 'dawidhaczela@gmail.com',//'nat@mywardrobehq.com', // put item.email here for live *** not in quotes, just item.email
    ...
    if(ss.length>0){
        ...
        netTotalS:netbalTotalS.toFixed(2),
